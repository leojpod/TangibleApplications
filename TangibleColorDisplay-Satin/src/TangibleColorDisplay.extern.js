...
var beforeunloadHandlers = new Array();
...
//  var exitOver = false;
  function init(step) {
    ...
    switch(step) {
      case 0:
        //first register
        tAPI.register('TangibleColorDisplay', "A SATIN component that reserve one cube a display colors on it",
          function(data) {
...
//      e_i_minus_1 = e_i;
//    }
//    console.log('while waiting for the requests to be done, I had time to compute fibo('+i+')='+e_i+' ...');
//  }
  //Return the compulsory SATIN structure!
  return {
    color: function(color) {
      tAPI.showColor(devId, color,
        function(){console.log('color displayed!');},
        function(data){console.log('woops : '+data.msg)});
    }
  };
}
window.onbeforeunload = function(e){
  var handler;
  while( (handler = beforeunloadHandlers.shift()) ){
    handler(e);
  }

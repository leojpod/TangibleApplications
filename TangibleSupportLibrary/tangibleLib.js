/*jslint bitwise: false, continue: false, debug: false, eqeq: false,
	...
	devel: true, indent: 4, maxlen: 90 */
/*global jQuery:false, WebSocket:false, window:false */
...
function TangibleAPI(server_ip) {
	...
	this.listDevices = function (onSuccess, onError, async) {
		if (appUUID === null) {
			...
			tangibleGET(svr_ip, appUUID + "/device/", {},
	...
	this.reserveDevice = function (deviceId, onSuccess, onError, async) {
		if (appUUID === null) {
			...
			if (reservationAttemptInProgress[deviceId] !== undefined) {
				...
				//so let's send an error message: 
				//  if the other reservation failed there will be a reason
				onError({ msg: "an attempt to get this device" +
					"is already running, try to get another one..."});
			} else {
				reservationAttemptInProgress[deviceId] = true;
//				console.log("about to make a call to reserve " + deviceId);
				tangiblePUT(svr_ip, 
						appUUID + "/device/reservation/" + deviceId, {},
					function (data) {
//						console.log("reservation successful");
	...
	this.releaseDevice = function (deviceId, onSuccess, onError, async) {
		if (appUUID === null) {
			...
			tangibleDELETE(svr_ip, 
					appUUID + "/device/reservation/" + deviceId, {},
				function (data) {
					var idx = -1, i;
					for (i = 0; 
							i < reservedDevices.length && idx === -1;
							i += 1) {
...
function SubscriptionMgr() {
	'use strict';
	var wsStream = null, listenerDict = [], isInit = false;

	function filterEvents(rcvEvt) {
//		console.log('about to filter:' + rcvEvt.data);
		var jsonMsg = jQuery.parseJSON(rcvEvt.data), devId, eventType, params = null, idx;
...
//			console.log('missing a required field, let\'s ignore the message');
	...
	this.init = function (appUUID, uri) {
		isInit = true;
//		console.log('SubscriptionMgr : init method called...');
		if (WebSocket === undefined) {
			throw "websocket are required to receive events from the devices!";
		}
...
//					console.log("device already reserved: " + labeledDevices[label]);
...
//					console.log("be patient  '" + label + "' is comming soon");
					comingSoonDevices[label].push(onUsable);
				} else {
					//otherwise, let's reserve it: 
					//we are the first one to require this device.
...
//							console.log('new added element ' + labeledDevices[label]);
							onUsable(data.msg);
							// we can warn the user and let him do his stuff with it
							//we have the device, it's time to wake up potential others
							if (comingSoonDevices[label] !== undefined) {
								while (
									(listener = comingSoonDevices[label].shift())
										!== undefined
								) {
						...
						//TODO create a reservation based on the type of devices 
						//TODO or on its capacity
						console.log("specifying deviceProperties not implemented yet!");
						onError({msg : "deviceProperties not implemented yet"});
					}
				}
			},
...
//console.log('tangibleLib loaded!');